...
const submitBtn = () =>{
	...
	if(checkIdFlag && checkNickNameFlag && checkPwdFlag && checkRePwdFlag){
		...
		if(!checkIdFlag){alert("아이디를 확인해주세요")}
		else if(!checkNickNameFlag){alert("닉네임을 확인해주세요")}
		else if(!checkPwdFlag){alert("비밀번호는 적어도 하나 이상의 숫자, 문자, 특수문자를 포함해야 합니다")}
		else if(!checkRePwdFlag){alert("비밀번호 확인란을 확인해주세요")}
	}
						...
						<input type="hidden" name="memEmail">
							...
							<div class="form-footer btn-box">
								<a class="btnset btnset-mono btnset-lg btnset-rect"
									...
									class="btnset2 btnset-lg btnset-rect" href="javascript:void(0)" onclick="submitBtn()">확인</a>
							</div>
						</form>
					</div>
				</div>
